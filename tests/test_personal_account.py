...
class TestPersonalAccount:
    def test_navigate_from_personal_account_to_constructor(self, driver_with_state):
    ...
    def test_navigate_from_personal_account_to_constructor_from_logo(self, driver_with_state):
        personal_account_button = driver_with_state.find_element(*GeneralLocators.PERSONAL_ACCOUNT_BUTTON)
        personal_account_button.click()
        WebDriverWait(driver_with_state, 5).until(EC.visibility_of_element_located(PersonalAccountLocator().SAVE_BUTTON))
    ...
    def test_logout_from_personal_account(self, driver_with_state):
        personal_account_button = driver_with_state.find_element(*GeneralLocators.PERSONAL_ACCOUNT_BUTTON)
        personal_account_button.click()
        WebDriverWait(driver_with_state, 5).until(EC.visibility_of_element_located(PersonalAccountLocator().SAVE_BUTTON))
